using Template.Domain.Contracts;
using Template.Domain.ValueObjects;
using Template.Domain.Contracts.Events;
using Template.Domain.Entities;

namespace Template.Domain.Events
{
    /// <summary>
    /// Represents a generic domain event that occurs when an entity is soft-deleted.
    /// </summary>
    /// <typeparam name="TKey">The type of the entity key.</typeparam>
    public record EntitySoftDeletedEvent<TEntity, TKey>(TKey EntityId, DateTime OccurredOn) : IDomainEvent, ISoftDeletedEvent<TEntity, TKey>
        where TEntity : ISoftDelete, IEntity<TKey>
        where TKey : struct, IIdentity<TKey>
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="EntitySoftDeletedEvent{TKey}"/> record.
        /// </summary>
        /// <param name="entityId">The unique identifier of the entity that was soft-deleted.</param>
        public EntitySoftDeletedEvent(TKey entityId) : this(entityId, DateTime.UtcNow) { }
    }
}
